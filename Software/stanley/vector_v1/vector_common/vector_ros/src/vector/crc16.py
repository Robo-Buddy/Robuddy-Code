...
"""
def compute_buffer_crc(byte_buffer):
    new_crc = 0
    ...
    byte_buffer.append((new_crc >> 8) & 0xFF)
    byte_buffer.append(new_crc & 0xFF)
    
"""
This computes the CRC for a buffer passed in and checks it against
the one contained in the buffer
"""
def buffer_crc_is_valid(byte_buffer):
    
    crc_index = len(byte_buffer) - 2
    new_crc = 0
    success = False
