...
class VectorMoveBase():
    def __init__(self):
        ...
        self.using_amcl = rospy.get_param("~using_amcl", True) #initially false
        self.global_frame = rospy.get_param("~global_frame", 'odom')
        self.base_frame = rospy.get_param("~base_frame", 'base_link')
        self.goal_timeout_sec = rospy.get_param("~goal_timeout_sec", 300)
        self.load_waypoints = rospy.get_param("~load_waypoints", True) #initially false. 
        self.waypoint_dwell_s= rospy.get_param("~waypoints_dwell_time", 5.0)
        self.run_waypoints = rospy.get_param("~run_waypoints", False) #initially false
        ...
        if (True == self.load_waypoints):
            ...
            for line in goalfile:
                ...
                rospy.loginfo("added another waypoint to waypoints[]")
            goalfile.close()
    ...
    def _run_waypoints(self):
        rospy.loginfo("beginning _run_waypoints")
        rospy.sleep(5)
        r = rospy.Rate(10)
        while not rospy.is_shutdown(): 
            if ((len(self.waypoints) > 0) and (self.present_waypoint < len(self.waypoints)) and (False == self.waypoint_is_executing) and (True == self.run_waypoints)):
                ...
                rospy.loginfo("about to send a goal")
                self._simple_goal_cb(goal)            
            self.marker_array_pub.publish(self.marker_array_msg)
            r.sleep()
    
    """ Adding a singular run to single waypoint from goals.txt file function -- Senior Design: RoBuddy""" 
    def _run_to_waypoint(self, goal_id):
        rospy.loginfo("beginning _run_to_waypoint... goal_id: " +  str( goal_id)) 
        rospy.sleep(5)
        r = rospy.Rate(10)
        if not rospy.is_shutdown():
            if ((len(self.waypoints) > 0) and (goal_id < len(self.waypoints)) and (False == self.waypoint_is_executing)):
                self.waypoint_is_executing = True
                goal = PoseStamped()
                goal.header.stamp = rospy.get_rostime()
                goal.header.frame_id = self.global_frame
                goal.pose = self.waypoints[goal_id]
                rospy.loginfo("about to send a goal")
                self._simple_goal_cb(goal)
    ...
    def _simple_goal_cb(self, simple_goal):
        ...
        rospy.loginfo("sending a goal to action server")
...
"""
def main():
    rospy.init_node('vector_move_base')
    uobotGoal = VectorMoveBase()
    robotGoal._run_waypoints()
